...
def main():
	...
	for itrain,itest in K_split_data:
		...
		stats = crossValidate(X_Train, Y_Train, X_Test, Y_Test, l_rate=1.2, epochs=200, reg_lambda=1e-1, params=[X_Train.shape[1],3,2,5], plot=True)
	...
	for itrain,itest in K_split_data:
		...
		stats = crossValidate(X_Train, Y_Train, X_Test, Y_Test, 2, 300, 1e-3, [X_Train.shape[1],3,2,2], True)
